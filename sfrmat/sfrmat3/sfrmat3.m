...
%  [status, dat, e, fitme, esf, nbin, del2] = sfrmat3(io, del, weight, a, oecfname);
...
for color=1:ncol                      % Loop for each color
    ...
    place = zeros(nlin,1);
    for n=1:nlin
        place(n) = fitme(color,2) + fitme(color,1)*(n-1);
